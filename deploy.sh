...
# ╔═══════════════════════════════════════════════════════════════════════════╗
# ║                          WDP-Help Deploy Script                            ║
# ║                     AI-Powered Help System Deployment                      ║
# ╚═══════════════════════════════════════════════════════════════════════════╝
...
if [ ! -f "pom.xml" ]; then
    print_error "Please run this script from the WDP-Help project directory"
    exit 1
fi
...
# Copy default configs if they don't exist; prompt before overwriting existing files
...
# Copy context directory (prompt before overwriting existing content)
print_step "Copying context files..."
mkdir -p "${CONFIG_DIR}/context"
if ask_overwrite "${CONFIG_DIR}/context" "Context directory exists: ${CONFIG_DIR}/context. Overwrite contents?"; then
    cp -r src/main/resources/context/* "${CONFIG_DIR}/context/"
    print_success "Context files copied"
else
    print_warning "Skipping overwriting context files"
fi
...
echo "  3. Configure API key in config.yml"
echo "  4. Test with /help command"
echo ""
echo -e "${YELLOW}⚠️  Remember to set your API key in config.yml!${NC}"
echo ""
